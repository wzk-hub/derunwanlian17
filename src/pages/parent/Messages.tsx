...
const ParentMessages = () => {
  ...
  const getChatGroupName = (group) => {
    // 查找群聊中的老师
    const teacherMember = group.members.find(m => m.role === 'teacher');
    return teacherMember ? `与${getUserName(teacherMember.id)}的群聊` : group.taskTitle;
  };
      ...
      {chatGroups.length === 0 ? (
        <div className="bg-white rounded-xl shadow-md p-12 text-center">
          ...
          <p className="text-gray-500 mb-6">
            当您选择老师并完成支付后，将创建与老师和客服的群聊
          </p>
          <button
            onClick={() => navigate('/parent/teachers')}
            className="px-6 py-2 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 transition-colors"
          >
            <i class="fa-solid fa-user-graduate mr-2"></i>选择老师
          </button>
        </div>
      ) : (
                ...
                <div className="p-4 border-t">
                  <div className="flex space-x-3">
                    <textarea
                      ...
                      placeholder="输入消息..."
                      className="flex-1 border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all resize-none"
                      rows={2}
                    ></textarea>
                    <button
